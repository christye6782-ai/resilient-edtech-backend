...
# Runtime override of settings.ollama_model (None = use the configured default).
# Lets the UI switch between a fast small model and a more capable one without
# a restart. Still fully offline — the model must already be pulled locally.
_runtime_model: Optional[str] = None

# Models the UI offers. label = what the teacher sees; id = the Ollama tag.
MODEL_CHOICES = [
    {"id": "llama3.2:1b", "label": "Fast (1B) — weakest devices"},
    {"id": "llama3.2:3b", "label": "Balanced (3B) — default"},
    {"id": "llama3.1:8b", "label": "Best quality (8B) — stronger BM"},
]
...
def get_model() -> str:
    """The active on-device model tag (runtime override or configured default)."""
    return _runtime_model or settings.ollama_model


def set_model(model: str) -> str:
    """Override the on-device model tag at runtime. Returns the tag now in effect."""
    global _runtime_model, _client
    model = (model or "").strip()
    if not model:
        raise ValueError("model tag must not be empty")
    _runtime_model = model
    _client = None  # not strictly needed (client is host-scoped) but keeps things clean
    return get_model()
...
def powered_by_label() -> str:
    ...
    if eng == "local":
        tag = get_model()
        pretty = tag.replace("llama", "Llama ").replace(":", " ").upper() if tag else ""
        return f"{tag} (on-device)"
    return "rule-based"
