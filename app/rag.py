...
def _ensure_table() -> None:
    ...
    # Migration for databases created before the subject column existed.
    cur.execute("PRAGMA table_info(rag_vectors)")
    cols = {r[1] for r in cur.fetchall()}
    if "subject" not in cols:
        cur.execute("ALTER TABLE rag_vectors ADD COLUMN subject TEXT")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_rag_coll ON rag_vectors(collection, teacher_id)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_rag_subject ON rag_vectors(collection, subject, year)")
    c.commit()
    c.close()
...
def _existing_hashes(collection: str) -> dict:
    """id -> content hash for everything already indexed in a collection.
    Lets index_curriculum() skip chunks whose text hasn't changed."""
    _ensure_table()
    c = dbmod.get_conn()
    cur = c.cursor()
...
def _upsert(row: dict) -> None:
    ...
    cur.execute(
        ...
        "(id, collection, teacher_id, subject, year, section, codes, text, hash, dim, embedding) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        (
            row["id"], row["collection"], row.get("teacher_id"), row.get("subject"),
            row.get("year"),
...
def _rows(collection: str, teacher_id: Optional[int] = None, year: Optional[int] = None,
    ...
    # Subject filter is SOFT: once several subjects are indexed we must not let
    # a Math lesson retrieve English standards. But if the filter matches
    # nothing (unknown subject, or a single-subject corpus), fall back to the
    # unfiltered set rather than returning no grounding at all.
    if subject:
        narrowed = [r for r in out if (r.get("subject") or subject) == subject]
        if narrowed:
            out = narrowed
    if year is not None:
        narrowed = [r for r in out if r.get("year") == year or r.get("year") is None]
        if narrowed:
            out = narrowed
    return out
...
def _load_rows(collection: str) -> list[dict]:
    ...
    cur = c.cursor()
    cur.execute(
        "SELECT id, teacher_id, year, section, codes, text, embedding, subject "
...
# Subject folders are named "<subject>_<level>", e.g. english_primary,
# math_primary, science_primary. The part before the first underscore is the
# canonical subject key stored on every chunk.
_SUBJECT_ALIASES = {
    "english": "english", "bahasa inggeris": "english", "bi": "english",
    "math": "math", "maths": "math", "mathematics": "math",
    "matematik": "math", "mt": "math",
    "science": "science", "sains": "science", "sn": "science",
    "malay": "malay", "bahasa melayu": "malay", "bm": "malay",
    "bahasa malaysia": "malay",
}


def _subject_key(raw: Optional[str]) -> Optional[str]:
    """Map a free-text subject ('Bahasa Inggeris', 'Maths') to a corpus key."""
...
def index_curriculum(force: bool = False) -> dict:
    ...
    Every subject folder under app/data/curriculum/ is picked up automatically;
    the folder name supplies the subject tag used to keep retrieval on-subject.
    """
    _ensure_table()
    if not embed_ready():
    ...
    for fp in files:
        if fp.name == "manifest.json":
            continue
        # folder name -> subject key (english_primary -> english)
        folder = fp.parent.name
        subject = _subject_key(folder.split("_")[0]) or folder.split("_")[0].lower()
        try:
        ...
        for rec in records:
            ...
            _upsert({
                ...
                "subject": rec.get("subject") or subject,
            ...
            indexed += 1
    return {"ok": True, "indexed": indexed, "skipped": skipped, "by_subject": by_subject}
...
def index_lesson(plan_id: int, teacher_id: int, text: str, meta: Optional[dict] = None) -> bool:
    ...
    _upsert({
        ...
        "subject": _subject_key(meta.get("subject")),
...
def retrieve(query: str, collection: str, k: int = 4,
             teacher_id: Optional[int] = None, year: Optional[int] = None,
             subject: Optional[str] = None) -> list[dict]:
    ...
    if not qv:
        return []
    scored = []
    for row in _rows(collection, teacher_id=teacher_id, year=year, subject=subject):
...
def curriculum_context(subject: Optional[str], form: Optional[str],
    ...
    hits = retrieve(query, "curriculum", k=k, year=year, subject=_subject_key(subject))
    if not hits:
        return ""
    lines = [
...
def status() -> dict:
    ...
    cur.execute(
        "SELECT COALESCE(subject,'(untagged)'), COUNT(*) FROM rag_vectors "
        "WHERE collection = 'curriculum' GROUP BY subject"
    )
    indexed_by_subject = {r[0]: r[1] for r in cur.fetchall()}
    c.close()
    # Every subject folder ships its own manifest; report them all.
    corpora = {}
    for mf in sorted(CORPUS_DIR.glob("*/manifest.json")):
        try:
            corpora[mf.parent.name] = json.loads(mf.read_text(encoding="utf-8"))
        except Exception:  # noqa: BLE001
            pass
    # Back-compat: `corpus` stays the English manifest for existing callers.
    manifest = corpora.get("english_primary", next(iter(corpora.values()), {}))
    return {
        "embed_model": EMBED_MODEL,
        "embed_ready": embed_ready(),
        "min_lessons_for_rag": MIN_LESSONS_FOR_RAG,
        "indexed": counts,
        "indexed_by_subject": indexed_by_subject,
        "subjects": sorted(corpora.keys()),
        "corpus": manifest,
        "corpora": corpora,
    }
