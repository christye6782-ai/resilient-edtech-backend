...
def analyse(req: AnalystRequest) -> AnalystResult:
    ...
    data = structured_call(_SYSTEM, user, _SCHEMA)
    if data is not None:
        try:
...
def _fallback(req: AnalystRequest) -> AnalystResult:
    ...
    alignments = [
        {
            ...
            "notes": (
                f"Lesson appears to address the topic '{matched_topic}'."
                if matched_topic
    ...
    return AnalystResult(
        summary=(
            ...
            + "Connect online or start Ollama (ollama pull llama3.2:3b) for a deeper, curriculum-aware review."
        ),
