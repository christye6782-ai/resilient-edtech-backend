"""ResilientEdTech — FastAPI backend (HYBRID build).

Run with:  uvicorn app.main:app --host 127.0.0.1 --port 5000 --reload
Then open: http://127.0.0.1:5000

What's new in the hybrid build:
  * /api/health now reports the active engine tier (cloud / on-device / basic),
    the selected mode, and per-tier availability.
  * GET/POST /api/engine lets the UI read and switch the engine mode at runtime
    (auto | cloud | local | basic) — this is what the top-bar AI-engine
    indicator binds to.
"""
...
_ENGINE_LABELS = {
    "cloud": f"Cloud AI ({settings.cloud_model})",
...
def health() -> dict:
    eng = engine_status()
    resolved = eng["resolved"]  # cloud | local | basic
    return {
        "status": "ok",
        "version": __version__,
        # Engine state
...
class EngineModeRequest(BaseModel):
    mode: str  # auto | cloud | local | basic
...
def post_engine(req: EngineModeRequest) -> dict:
    """Switch the engine mode at runtime (auto | cloud | local | basic)."""
    try:
        set_mode(req.mode)
