...
def answer(req: FaqRequest) -> FaqResult:
    ...
    data = structured_call(_SYSTEM, user, _SCHEMA, max_tokens=1500, lang=getattr(req, "lang", "en"))
    if data is not None:
        try:
...
def _fallback(req: FaqRequest) -> FaqResult:
    ...
    for tool in tools["tools"]:
        if tool["name"].lower() in q:
            if ms:
                notes = (f"Internet: {tool.get('needs_internet', 'tiada')} | "
                         f"Peranti: {tool.get('devices_required', 'tiada')}")
                alt = [tool.get("offline_alternative", "Guna kaedah setara berasaskan kertas.")]
            else:
                notes = (f"Internet: {tool.get('needs_internet', 'n/a')} | "
                         f"Devices: {tool.get('devices_required', 'n/a')}")
                alt = [tool.get("offline_alternative", "Use a paper-based equivalent.")]
            return FaqResult(
                term=tool["name"],
                explanation=f"{tool['what']} ({tool['category']}).",
                how_to_execute=tool.get("how_to_use", []),
                low_resource_notes=notes,
                alternatives=alt,
    ...
    if ms:
        return FaqResult(
            term=req.question.strip()[:60] or "Istilah",
            explanation=(
                "Tiada takrifan terbina untuk istilah ini. Mulakan model atas peranti "
                "(ollama pull llama3.2:3b) untuk penjelasan penuh yang sedar-konteks — masih sepenuhnya luar talian."
            ),
            how_to_execute=["Cari laman rasmi alat itu semasa ada internet, kemudian catat langkah-langkah untuk kegunaan luar talian."],
            low_resource_notes="Apabila tidak pasti, utamakan alat yang hanya perlukan satu peranti guru dan tiada internet murid.",
            alternatives=["Lembaran kerja bercetak atau bahan manipulatif biasanya mencapai matlamat pembelajaran yang sama."],
            powered_by="rule-based",
        )
    ...
    return FaqResult(
        ...
        explanation=(
            "No built-in definition found for this term. Start the on-device model "
            "(ollama pull llama3.2:3b) for a full, context-aware explanation — still fully offline."
        ),
